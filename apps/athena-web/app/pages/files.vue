...
const { t } = useI18n()
const { fetchFiles, fetchUsage, deleteFile, downloadFile, formatBytes } = useMedia()
...
const { data: filesData, status: filesStatus, refresh: refreshFiles } = await fetchFiles(filters)

const { data: usageData, refresh: refreshUsage } = await fetchUsage()

const files = computed(() => filesData.value?.data || [])
const total = computed(() => filesData.value?.meta?.total || 0)
const loading = computed(() => filesStatus.value === 'pending')

const usage = computed(() => usageData.value || { usedBytes: 0, limitBytes: 0, percentage: 0 })
const usageColor = computed(() => {
  if (usage.value.percentage >= 90) return 'error'
  if (usage.value.percentage >= 75) return 'warning'
  return 'primary'
})

const columns = computed<TableColumn<FileResponse>[]>(() => [
  { accessorKey: 'originalName', header: t('pages.files.columns.name'), class: 'w-[40%]' },
  { accessorKey: 'access', header: t('pages.files.columns.access') },
  { accessorKey: 'mimeType', header: t('pages.files.columns.type') },
  { accessorKey: 'size', header: t('pages.files.columns.size') },
  { accessorKey: 'createdAt', header: t('pages.files.columns.created') },
  { id: 'actions', header: '' }
])
...
const refreshAll = async () => {
  await Promise.all([refreshFiles(), refreshUsage()])
}
  ...
  <div class="p-4 space-y-6">
      ...
      <UCard>
        <div class="space-y-3">
          ...
          <div class="flex justify-between text-xs text-gray-500">
            ...
            <span>{{ $t('common.of') }} {{ formatBytes(usage.limitBytes) }}</span>
          </div>
        </div>
      </UCard>
    </div>
    ...
    <div class="flex flex-col sm:flex-row justify-between gap-4">
      <UInput
        ...
        :placeholder="$t('pages.files.search')"
